...
export default function VendasPorMovimento() {
  ...
  useEffect(() => {
    async function fetchProdutosComVenda() {
      try {
        ...
        const vendas = json.filter(item => item.Tipo === "Venda");

        const produtosUnicos = Array.from(
          new Map(vendas.map(p => [p["ID Produto"], p])).values()
        );
  ...
  return (
    <div className="space-y-4 w-full">
      <h2 className="text-xl font-bold border-2 rounded-2xl py-2 text-center">📊 Vendas por Movimento</h2>
      ...
      <div className="flex flex-col md:flex-row gap-6 items-center">
        ...
        <div className="relative w-80" ref={dropdownRef}>
          ...
          {dropdownAberto && (
            <div className="absolute bg-white dark:bg-gray-800 border mt-1 rounded shadow w-full max-h-60 overflow-y-auto z-10 p-2">
              {carregando ? (
                <p className="text-gray-500 dark:text-gray-200 text-sm text-center py-2">
                  Carregando produtos...
                </p>
              ) : (
                <>
